...
*Sync, customize, and natively record breathtaking lyric videos directly in your browser.*
...
- [📸 Step-by-Step Visual Guide](#-step-by-step-visual-guide-how-to-use)
...
```text
Main abhi jo YouTube video dekh raha hu, uske gaane ke pure lyrics Hinglish (English alphabet mein Hindi) mein nikal kar do. 
"youtubeUrl" field mein isi video ka exact link (URL) nikal kar daalna, apna koi link mat banana.
...
```text
Main abhi jo YouTube video dekh raha hu, uske gaane ke pure lyrics Hinglish mein nikal kar do. 
"youtubeUrl" field mein isi video ka exact link (URL) nikal kar daalna, apna koi link mat banana.
Mujhe output ek valid JSON format mein chahiye, bilkul is example ki tarah:
...
```text
Main abhi jo YouTube video dekh raha hu, uske gaane ke pure lyrics Hindi font (Devanagari) mein nikal kar do. 
"youtubeUrl" field mein isi video ka exact link (URL) nikal kar daalna, apna koi link mat banana.
Mujhe output ek valid JSON format mein chahiye, bilkul is example ki tarah:
...
**For English Lyrics**
```text
Extract the full lyrics of the YouTube video song I am currently watching, in English.
In the "youtubeUrl" field, extract and insert the exact URL of this current video. Do not hallucinate or search for another link.
...
Built with ❤️ by **Sudhir** (Copyright © 2026).  
...
- 🎵 **Live Demo**: [song-and-lyrics.vercel.app](https://song-and-lyrics.vercel.app/)

---

## 🙏 Acknowledgments & Credits

This project was made possible by leveraging the following incredible open-source APIs and technologies:

* **[LRCLib](https://lrclib.net/)**: For providing an exceptional, free API to auto-fetch time-synced `.lrc` lyrics.
* **[YouTube IFrame Player API](https://developers.google.com/youtube/iframe_api_reference)**: For enabling seamless background video streaming and audio playback.
* **[Google Fonts](https://fonts.google.com/)**: For providing the beautiful typography (Outfit, Poppins, and Yatra One for Hindi text).
* **Web APIs**: Leveraging modern browser capabilities including the `MediaRecorder`, `Tab Capture API` (`getDisplayMedia`), and `requestAnimationFrame` for 60FPS rendering and native video export.
