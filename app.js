...
(function () {
    ...
    function parseLyrics(text) {
        ...
        const res = [];
        const re = /^\[(\d+):([\d.]+)\]\s*(.*)/; // supports [0:05] and [0:05.5]
        const hasTm = lines.some(l=>re.test(l));
        if (hasTm) {
            lines.forEach(l => { const m=l.match(re); if(m) res.push({time: +m[1]*60 + parseFloat(m[2]), text:m[3].trim()}); });
        } else {
            const gap = SPEED[S.speed]||3;
            lines.forEach((l,i) => res.push({time:i*gap,text:l.trim()}));
        }
        return res;
    }
